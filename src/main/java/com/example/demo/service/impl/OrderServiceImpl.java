...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderEntity processCheckout(CheckoutRequest request) {
        ...
        ShippingEntity shipping = shippingRepository.findById(request.getShippingId())
        ...
        for (CheckoutRequest.CheckoutItem item : request.getItems()) {
            ...
            orderItem.setPrice(product.getPriceOriginal()); // giá tại thời điểm mua
            orderItem.setProductSize(item.getProductSize());
            orderItem.setProductColor(item.getProductColor());

            totalQuantity += item.getQuantity();
            totalAmount = totalAmount.add(
                    product.getPriceOriginal().multiply(BigDecimal.valueOf(item.getQuantity())));
